...
def Test_pre_sam(args, val_loader, epoch, net: nn.Module, clean_dir=True):
    # eval mode
    net.eval()
...
def Test_pre_no_sam(args, val_loader, epoch, net: nn.Module, clean_dir=True):
    # eval mode
    net.eval()
    ...
    for ind, pack in enumerate(val_loader):
        ...
        while (buoy + evl_ch) <= imgsw.size(-1):
            ...
            with torch.no_grad():
                ...
                pred = torch.clamp(masks, min=0, max=0)
                tot += lossfunc(pred, masks)
...
def Predict_sam(args, val_loader, epoch, net: nn.Module, clean_dir=True):
        ...
        for ind, pack in enumerate(val_loader):
            ...
            while (buoy + evl_ch) <= imgsw.size(-1):
                ...
                with torch.no_grad():
                    ...
                    masks_np = Image.fromarray(((masks_np[0] - 255) * (-1)).astype(np.uint8))
    ...
    return masks_np, image, img_name

def Predict_no_sam(args, val_loader, epoch, net: nn.Module, clean_dir=True):
    # eval mode
    net.eval()
        ...
        for ind, pack in enumerate(val_loader):
            ...
            while (buoy + evl_ch) <= imgsw.size(-1):
                ...
                with torch.no_grad():
                    ...
                    pred = torch.clamp(masks, min=255, max=255)
                    # tot += lossfunc(pred, masks)
                    ...
                    if ind % args.vis == 0:
                        ...
                        # if args.prompt_approach == 'random_click':
                        #     vis_image(imgs, pred, pred, os.path.join(args.path_helper['test_sample_path'],
                        #                                               namecat + 'epoch+' + str(epoch) + '.jpg'),
                        #               reverse=False, points=showp)
                        # elif args.prompt_approach == 'points_grids':
                        #     print(namecat + 'epoch+' + str(epoch) + '.jpg')
                    ...
                    #############################capture_mask_image and save###########################
                    h, w = masks_np.shape[-2:]
                    # masks_np = masks_np.reshape(1, h, w)

                    masks_np = Image.fromarray(((masks_np[0] - 255) * (-1)).astype(np.uint8))
                    # masks_np.save('/home/zhanghuishen/Desktop/endoscope/final/fusion-sam/mask_save/' + img_name + '.png')
                    image = Image.fromarray(np.uint8(image * 255))
                    # image.save('/home/zhanghuishen/Desktop/endoscope/final/fusion-sam/image_save/' + img_name + '.jpg')
                    ##################################################################################
                    # show_img_mask(image, masks_np, img_name)

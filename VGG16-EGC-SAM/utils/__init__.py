...
def iou(outputs: np.array, labels: np.array):
    ...
    precision = (intersection + SMOOTH) / (outputs.sum((1, 2)) + SMOOTH)
    recall = (intersection + SMOOTH) / (labels.sum((1, 2)) + SMOOTH)

    return iou.mean(), precision.mean(), recall.mean()
...
def eval_seg(pred, true_mask_p, threshold):
    ...
    if c == 2:
        ...
        eiou, precision, recall, edice, eiou_, precision_, recall_ = 0, 0, 0, 0, 0, 0, 0
        for th in threshold:
            gt_vmask_p = (true_mask_p > 0.5).float()
            vpred = (pred > 0.5).float()
            vpred_cpu = vpred.cpu()
            disc_pred = vpred_cpu[:, 0, :, :].numpy().astype('int32')
            ...
            '''iou for numpy'''
            eiou_, precision_, recall_ = iou(disc_pred, disc_mask)
            eiou += eiou_
        ...
        return eiou / len(threshold), edice / len(threshold), precision / len(threshold), recall / len(threshold)

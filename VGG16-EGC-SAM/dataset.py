...
class Cryoppp_test_pre_Dataset(Dataset):
    def __init__(self, args, data_path, file_name, transform=None, transform_msk=None, mode='train', prompt='random_click',
                 plane=False):

        # label_list, name_list = get_imagse_and_labels_path(data_path, mode)

        label_list = [os.path.join(data_path, mode, "labels", file_name + '.png')]
        name_list = [os.path.join(data_path, mode, "images", file_name + '.jpg')]
...
def get_imagse_path(data_path, mode):

    # label_list = sorted([os.path.join(data_path, mode, "labels", label_file) for label_file in os.listdir(os.path.join(data_path, mode, "labels"))])
    image_list = sorted([os.path.join(data_path, mode, "images", image_file) for image_file in os.listdir(os.path.join(data_path, mode, "images"))])

    print(mode, "data length:", len(image_list))

    return image_list

class CryopppDataset_pre(Dataset):
    def __init__(self, args, data_path, transform=None, transform_msk=None, mode='train', prompt='random_click',
                 plane=False):

        name_list = [os.path.join(data_path)]
